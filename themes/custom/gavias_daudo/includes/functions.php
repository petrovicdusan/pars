<?php
function gavias_daudo_render_block($key) {
...
function gavias_daudo_makeid($length = 5){
...
function gavias_daudo_base_url(){
  global $base_url;
  $theme_path = drupal_get_path('theme', 'gavias_daudo');
  return $base_url . '/' . $theme_path . '/';
}

global $gva_node_index;
function gavias_daudo_preprocess_node(&$variables) {
  global $gva_node_index;
...
function gavias_daudo_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function gavias_daudo_preprocess_breadcrumb(&$variables){
...
function gavias_daudo_print_icon($file, $alt){
